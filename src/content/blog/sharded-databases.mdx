---
title: "Sharded Databases: What They Are and Why They Matter"
...
Thus, queries are executed faster and the efficiency of the systems is maintained. 
...
An example of this is a online shopping website which could use customer ID as it's a shard key. 
This means all of a customer's orders and interactions get saved on a particular shard. 
That way when a user wants to sign in and look at their purchase history the system doesn't have to search through millions of records it just has to look at the shard related to that user which is much faster.
Because let's be honest, no one wants to wait 30 seconds for their shopping cart to load, especially during a Black Friday sale!.
...
Sharding is most useful in applications that deal with a vast amount of data and many users. Some common use cases include: 

 - Social media sites (Facebook, Twitter): Billions of users create data in real time. 
 - E-commerce sites (Amazon, eBay): All orders and transactions are required to be processed as quickly as possible. 
 - Banking and financial networks: There are many secure and timely transactions that are required. 
 - Multiplayer online games: All the data related to players, their scores, and interactions needs to be saved and accessed in real time. 
 
If your database is facing slow queries, high storage expenses, or down time, then sharding may be worthy of a thought. 
But if a system can do fine with caching or indexing, then sharding may bring in more complexity than needed.

## Final Thoughts

Sharding is a great way for handling large scale and high traffic applications but it is not for everyone. 
However, it is worth noting that it enhances scalability and performance at the expense of complexity and maintenance. 
Before implementing sharding, businesses should evaluate their needs carefully and determine if there are other solutions (e.g., indexing, caching, or replication) that could be used instead. 

If done right, sharding can be the lifesaver that allows organizations to future proof their databases, to grow without sacrificing speed or reliability.
